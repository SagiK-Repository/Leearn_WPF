...
WPF의 MVVM에 대해 이해할 수 있는 내용이었습니다.  
앞으로 [+WPF의 MVVM에 대해서 다룰 때+] [-이번에 활용한 예제를 많이 참고 할 것-]입니다.  
특히 프로젝트 생성부터 [+모든 과정 하나하나에 자세한 설명과 그 과정을 자세히 명시해 놓았기에+], 정말 [-큰 도움-]이 되었습니다.  
또한 DevExpress 및 ReactiveUI에 대해서 사용법을 익힐 수 있었는데, 이들의 사용법을 보다 자세히 공부하고 가능하면 다른 Nuget들에 대해서도 공부해보고자 합니다.  
그리고 WPF의 MVVM 구조를 보다 자세히 알아가기 위해, 이 예제를 중심으로 [+개인적으로도 WPF MVVM 프로젝트를 만들어 볼려고 합니다.+]  
유튜브에 있는 MVVM에 대한 [-수 많은 다른 강의보다 훨 씬 유익-]했던 교육인 것 같습니다.  
WPF에 대해서 잘 제어하고 발전된 MVVM 구조를 추구하여 원하는대로 목표한 것을 만들어낼 수 있도록, 개인적으로 WPF MVVM 관련 프로그램 및 타 교육을 받아보면서 WPF MVVM에 대해 더욱 알아가는 제가 되겠습니다.  

  <br>

교육을 받으면서 몇 가지 [+아쉬운점+]이 있었습니다.  
우선 [+첫 번째 챕터+]는 MVVM 구조를 배우기 전 Binding을 활용한 프로젝트인데, ppt가 MVVM 중심의 PPT이다 보니 이 부분에 [-생략한게 많은-] [+옥의 티+]가 있었습니다.  
처음 배우는 입장에서 [-생략된 코드-]들을 만나 조금 헤메인적이 여간 한 번이 아닙니다. 하지만 그 부분은 앞으로 ppt를 중심삼고 공부하려는 사람들에게 조금 도움이 될 수 있도록 [+ppt내용을 몇 가지 수정+]하였습니다.  
다음은 ppt 수정 내용입니다.  

### ppt 수정사항

- P.13 Chapter0 이미지 부연설명 추가
- P.15 Chapter0 SelectedIndex 추가
- P.27 Chapter1 TextBox Binding 추가
- P.40 Chapter1 가독성을 위한 이미지 위치 수정
- P.49 Chapter2 Expression.Blend.Sdk활용 보조 설명 추가
- P.65~71 Chapter2 코드 보조 설명 추가
- P.79 Chapter3 코드 보조 설명 추가

  <br>

  확인해주시면 [MVVM 관련 자료](http://wish.mirero.co.kr/mirero/withcommunity/-/issues/56)에 수정 사항 추가하겠습니다.  
  [4.WPF.Education_20221005.pptx📎](/uploads/c37e4d4a47692776f84c240edba4636c/4.WPF.Education_20221005.pptx)<br>

  <br>

이번에도 교육할 때 시간 소요를 관리하면서 공부를 하였습니다. 다음과 같이 시간소요가 되었습니다.  
...
- 한 챕터당 평균 3h 8m (187.5m) 소요
- 소요시간 : 12.5h, 하루 8h 소요하면 학습시간은 1.56d 소요했습니다.
- 기술 세미나나 다른 특별 시간을 제외해서 고려하였기에, [+2~3d까지 소요할 수 있을 것+]으로 보입니다.

  <br>

목표했던 하루는 이루지 못했지만, 내용을 자세히 이해하려고 하다보니 시간이 미루어 진 것 같습니다.  
Chapter01에서 조금 헤매서 오래 걸렸고, Chapter 2~3는 하나하나 공부하면서 교육하였기에 오래 걸린 것 같습니다.  
앞으로 다른 교육에 있어서도 목적에 맞게 교육하겠습니다!
